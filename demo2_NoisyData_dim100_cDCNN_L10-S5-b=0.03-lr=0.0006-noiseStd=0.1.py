...
from regression_pipeline_cDCNNs  import RegressionWithContractDeepConvolutionalNeuralNetworkSingleChannelsInitialBias
...
if __name__ == '__main__':
    ...
    # add logs
    fig_save_path = './newlogs_noisydata/cdcnn-smoothDim100/'
    if not os.path.exists(fig_save_path): os.makedirs(fig_save_path)
    file_name = (
        f'cDCNN_Ntr={train_sample_size}_Nte={test_sample_size}_num_layers={num_layers}_filter_size={filter_size}_channels={num_channels}'
        f'_pooling_size={pooling_size}_epoch={num_epoch}_b={bias_value}_lr={learning_rate}')
    ...
    for row_idx in range(len(bias_value_list)):
        for col_idx in range(len(learning_rate_list)):
            ...
            for ex_iter in tqdm(range(repeated_times)):
                ...
                eDCNN_zeropadding_network = RegressionWithContractDeepConvolutionalNeuralNetworkSingleChannelsInitialBias(
                    X_train=x_train,
                    Y_train=y_train,
                    X_test=x_test,
                    Y_test=y_test,
