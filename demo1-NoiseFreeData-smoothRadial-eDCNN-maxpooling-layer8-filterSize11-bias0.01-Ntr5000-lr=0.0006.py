...
if __name__ == '__main__':
    ...
    repeated_times = 10  # the number of individual runs
    ...
    filter_size = 11  # the filter length of Conv1d layer
    num_channels = 1  # single-channel network
    pooling_size = 2
    ...
    # add logs
    fig_save_path = './newlogs/eDCNN-maxpooling-smoothRadial-dim4/'
    if not os.path.exists(fig_save_path): os.makedirs(fig_save_path)
    file_name = (
        f'eDCNN-maxpooling_Ntr={train_sample_size}_num_layers={num_layers}_filter_size={filter_size}_num_channels={num_channels}'
        f'_pooling_size={pooling_size}_num_epoch={num_epoch}')

    logger.add(fig_save_path + f"{file_name}.log", enqueue=True)
    # ----------------------------------------------------------------
    logger.info(file_name)

    bias_value_list = [ 0.01]
